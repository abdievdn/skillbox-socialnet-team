  ...
  .comment-main
    ...
    template(v-else)
      .edit.edit--small(v-if="edit || deleted")
        button.edit__icon.edit__icon--remove(v-if="deleted" @click="onDeleteComment")
          simple-svg(:filepath="'/static/img/delete-news.svg'")
        button.edit__icon.edit__icon--edit(v-if="edit" @click="editComment")
      ...
      .comment-main__main
        ...
        .comment-main__actions
          ...
          template(v-if="!admin")
            ...
            like-comment(fill :active="info.my_like" :quantity="info.likes" :id="info.id" @liked="likeAction" )
</template>
